...
export function ValueSection({ id, eyebrow, heading, body, columns, zIndex }: ValueSectionProps) {
  return (
    <SectionContainer id={id} zIndex={zIndex} className="overflow-visible">
      <div className="grid h-full items-center gap-10 lg:grid-cols-[minmax(0,0.92fr)_minmax(0,1.08fr)] lg:gap-14">
        ...
        <div className="relative">
          ...
          <div className="flex flex-col gap-4 lg:flex-row lg:items-stretch lg:gap-3">
            {columns.map((col, i) => (
              <div key={col.title} className="flex flex-col items-center gap-3 lg:flex-1 lg:flex-row lg:gap-3">
                <RevealWrapper delay={i * 0.1}>
                  <article className="relative w-full rounded-[1.4rem] border border-border/80 bg-card/70 p-5 shadow-[0_22px_60px_-38px_rgba(15,23,42,0.3)] backdrop-blur-sm lg:min-h-[16.5rem] lg:p-6">
